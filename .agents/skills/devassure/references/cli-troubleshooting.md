...
On auth-related CLI errors, run `devassure ping --no-ui` to verify connectivity and token status. If unhealthy, re-login with `devassure login --confirm --no-ui` or run `devassure login` in your terminal.
